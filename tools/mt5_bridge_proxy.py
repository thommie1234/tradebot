...
_MIN_REQUEST_INTERVAL = 0.01     # 10ms minimum between requests (100 req/s max)
...
class MT5Handler(socketserver.StreamRequestHandler):
    def handle(self):
        ...
        # Throttle: wait if requests are coming too fast
        with _lock:
            now = time.time()
...
class _ReusableTCPServer(socketserver.ThreadingTCPServer):
    ...
    daemon_threads = True
    request_queue_size = 32  # backlog for pending connections
