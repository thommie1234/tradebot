...
SESSION_FILE_BASE = str(REPO_ROOT / "tools" / "telegram_session")
    ...
    try:
        ...
        _defaults = {
            ...
            "gold": {"atr_timeframe": "M15", "atr_period": 14, "atr_sl_mult": 0.8,
                     ...
                     "trail_distance_atr": 0.20, "risk_per_trade": 0.003, "magic_number": 5001},
        ...
        # Daily loss check — block if daily loss > 2%
        try:
            import yaml as _yaml
            _accts_path = REPO_ROOT / "config" / "accounts.yaml"
            _day_start = account_info.balance  # approximation
            if _accts_path.exists():
                with open(_accts_path) as _af:
                    _accts = _yaml.safe_load(_af).get("accounts", {})
                _acfg = _accts.get(account_id, {})
                _max_daily = _acfg.get("internal_daily_loss_pct", 0.02)
            else:
                _max_daily = 0.02
            _daily_pnl = (account_info.equity - _day_start) / _day_start if _day_start > 0 else 0
            if _daily_pnl <= -_max_daily:
                return False, f"Daily loss {_daily_pnl:.2%} hit -{_max_daily:.0%} — blocked"
        except Exception:
            pass

        # Load per-account risk from config JSON
        _acct_risk = None
        try:
            import json as _json
            _cfg_map = {"bright_100k": "bf/config.json", "ftmo_100k": "ftmo/config.json"}
            _cfg_path = REPO_ROOT / _cfg_map.get(account_id, "")
            if _cfg_path.exists():
                with open(_cfg_path) as _f:
                    _acct_cfg = _json.load(_f)
                if signal.symbol in _acct_cfg and isinstance(_acct_cfg[signal.symbol], dict):
                    _acct_risk = _acct_cfg[signal.symbol].get("risk_per_trade")
        except Exception:
            pass
        risk_pct = _acct_risk if _acct_risk is not None else sym_cfg.get("risk_per_trade", 0.003)
        ...
        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
            ...
            try:
                ...
                _discord_cfg = REPO_ROOT / "tools" / "discord_config.json"
                if _discord_cfg.exists():
                    with open(_discord_cfg) as _df:
                        _dcfg = _djson.load(_df)
                    _webhook = _dcfg.get("webhook_url", "")
                    if _webhook:
                        _dn = DiscordNotifier(_webhook)
                        _acct_tag = "BF" if "bright" in account_id else "FTMO"
                        _dn.send(
                            f"TELEGRAM SIGNAL — {signal.direction} {signal.symbol}",
                            f"Channel: {signal.channel}\n"
                            f"Lots: {lot_size:.2f} @ {entry_price:.5f}\n"
                            f"SL: {sl_price:.5f} | TP: {tp_price:.5f}\n"
                            f"Risk: {risk_pct*100:.2f}% | Account: {_acct_tag}\n"
...
async def run_scraper(channels: list = None, account_id: str = "bright_100k",
    ...
    # Per-account session file to avoid SQLite lock between BF and FTMO scrapers
    acct_tag = account_id if isinstance(account_id, str) else account_id[0]
    session_file = f"{SESSION_FILE_BASE}_{acct_tag}"
