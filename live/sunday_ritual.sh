...
WINE_LAUNCHER="live/run_wine.sh"
WORKERS=6
SPLIT_DIR="tools/symbol_splits"
mkdir -p "$SPLIT_DIR"
"$VENV" "tools/make_symbol_splits.py" --workers "$WORKERS" --out-dir "$SPLIT_DIR" 2>&1 | tee -a "$LOG_FILE"
...
(
  ...
  CUDA_VISIBLE_DEVICES=0 "$VENV" -u "research/optuna_orchestrator.py" \
...
(
  ...
  CUDA_VISIBLE_DEVICES=1 "$VENV" -u "research/optuna_orchestrator.py" \
    --all --trials $TRIALS --discovery-cutoff $DISCOVERY \
    --train-size $TRAIN_SIZE --test-size $TEST_SIZE \
    --purge $PURGE --embargo $EMBARGO \
    --pt-mult 2.0 --sl-mult 1.5 --horizon-bars 24 --z-threshold 1.0 \
...
if [ "$SURVIVORS" -eq 0 ]; then
  ...
  CUDA_VISIBLE_DEVICES=0 OPTUNA_CSV_OVERRIDE="$(realpath "$COMBINED_CSV")" \
    "$VENV" -u "live/run_bot.py" --retrain 2>&1 | tail -30 | \
    tee -a "$LOG_FILE"
  ...
  "$VENV" -u "research/exit_optuna.py" \
...
CUDA_VISIBLE_DEVICES=0 "$VENV" -u "tools/llm_trade_advisor.py" --once 2>&1 | \
...
CUDA_VISIBLE_DEVICES=0 "$VENV" -u "analysis/monte_carlo.py" \
