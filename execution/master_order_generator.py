...
class AccountSnapshot:
    ...
    def from_account(cls, acct) -> 'AccountSnapshot':
        ...
        positions_raw = (acct.mt5.positions_get() if connected else None) or ()

        # Filter: positions_get() can return ints on error
        positions_list = []
        ...
        # DrawdownGuard calculates day_pnl on-the-fly, no stored attribute
        day_pnl = 0.0
        if acct.drawdown_guard and connected:
            try:
                base = acct.drawdown_guard._day_start_balance
                if base and base > 0:
                    day_pnl = (account_info.equity - base) / base
            except Exception:
                pass
    ...
    def _evaluate_signal_for_account(self, signal: Signal,
        ...
        if snap.safe_mode:
            self.logger.log('DEBUG', 'MasterGen', 'REJECTED',
                            f'[{aid}] {signal.symbol}: safe_mode active')
            return None

        if snap.emergency_stop:
            self.logger.log('DEBUG', 'MasterGen', 'REJECTED',
                            f'[{aid}] {signal.symbol}: emergency_stop active')
            return None
        ...
        if snap._account_symbols and broker_sym not in snap._account_symbols:
            self.logger.log('DEBUG', 'MasterGen', 'REJECTED',
                            f'[{aid}] {signal.symbol}: symbol not in account config')
            return None
        ...
        if router is None:
            self.logger.log('DEBUG', 'MasterGen', 'REJECTED',
                            f'[{aid}] {signal.symbol}: no order_router available')
            return None
