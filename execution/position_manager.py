...
class PositionManager:
    ...
    def __init__(self, logger, mt5, discord=None):
        self.logger = logger
        self.mt5 = mt5
    ...
    def manage_positions(self, running: bool, emergency_stop: bool):
        ...
        try:
            ...
            self.logger.log('ERROR', 'PositionManager', 'MANAGE_ERROR', str(e))
    ...
    def _process_single_position(self, pos):
        ...
        sym_cfg = cfg.SYMBOLS.get(pos.symbol, {})
            ...
            for pos in positions:
                ...
                if result and result.retcode == self.mt5.TRADE_RETCODE_DONE:
                    ...
                    self.logger.log('INFO', 'HorizonExit', 'POSITION_CLOSED',
                                    ...
                                    f'PnL={pnl_str}')
                    if self.discord:
                        color = "orange" if (real_pnl or pnl) >= 0 else "red"
                        self.discord.send(
                            f"HORIZON EXIT: {pos.symbol}",
    ...
    def auto_close_bleeders(self, running: bool, emergency_stop: bool):
        ...
        try:
            ...
            for pos in positions:
                ...
                if hold_hours > 48 and abs(pnl) < 5.0:
                    reason = f"ZOMBIE (pnl=${pnl:+.2f}, {hold_hours:.0f}h held, dead capital)"

                if hold_hours > 24 and pnl < -50:
        ...
        try:
            ...
            for pos in positions:
                ...
                if state["strikes"] >= ML_EXIT_STRIKES:
                    pnl = pos.profit + pos.swap
                    self._ml_exit_close(pos, proba, state["strikes"], pnl)
                    state["strikes"] = 0
    ...
    def _ml_exit_close(self, pos, proba: float, strikes: int, pnl: float):
        """Close a position because ML model no longer supports it."""
        tick = self.mt5.symbol_info_tick(pos.symbol)
        if not tick:
            return
