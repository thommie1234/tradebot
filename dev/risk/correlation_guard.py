...
CRYPTO_SYMBOLS = {
    ...
    'SOLUSD', 'DOGEUSD', 'DOTUSD', 'BNBUSD',
    'AAVUSD', 'UNIUSD', 'ETCUSD', 'ICPUSD',
    # Broker-specific suffixes handled by _normalise()
}

METAL_SYMBOLS = {'XAUUSD', 'XAGUSD', 'GOLDUSD', 'SILVERUSD'}
OIL_SYMBOLS = {'USOIL', 'UKOIL', 'WTIUSD', 'BRENTUSD', 'CRUDEOIL'}

INDEX_REGIONS: dict[str, set[str]] = {
    'us':   {'US30', 'US100', 'US500', 'USTEC', 'US2000', 'SP500', 'NAS100', 'DJ30'},
    'eu':   {'DE40', 'GER40', 'FR40', 'FRA40', 'EU50', 'STOXX50', 'DAX40', 'CAC40'},
    'uk':   {'UK100', 'FTSE100'},
    'apac': {'JP225', 'AU200', 'AUS200', 'HK50', 'CN50', 'NI225'},
}

EQUITY_SECTORS: dict[str, set[str]] = {
    'us_equity': {'AAPL', 'MSFT', 'GOOGL', 'GOOG', 'AMZN', 'TSLA', 'NVDA', 'META', 'NFLX',
                  'V', 'BAC', 'PFE', 'ZM', 'BABA'},
    'eu_equity': {'SAP', 'ASML', 'LVMH', 'SIE', 'BMW', 'DBKGn', 'ALVG', 'RACE'},
}

# ── Limits ───────────────────────────────────────────────────────────
MAX_CURRENCY_SAME_DIR = 3    # forex: max 3 positions per currency in same direction
MAX_CRYPTO_SAME_DIR = 3      # all crypto treated as one correlated group (3 crypto symbols)
MAX_METAL_SAME_DIR = 2       # gold + silver correlated
MAX_INDEX_SAME_DIR = 2       # per regional group
MAX_EQUITY_SAME_DIR = 4      # per equity sector (larger portfolio)


def _normalise(symbol: str) -> str:
    """Strip common broker suffixes (.r, .i, .cash, _SB, etc.)."""
    for suffix in ('.r', '.i', '.e', '.cash', '_SB', '.ecn', '.pro'):
...
class CorrelationGuard:
    ...
    def check_correlation(self, symbol: str, direction: str,
        ...
        if asset_class == 'commodity' or sym in METAL_SYMBOLS or sym in OIL_SYMBOLS:
    ...
    def _check_forex(self, sym: str, direction: str, positions) -> bool:
        ...
        # Deduplicate split orders: (symbol, type, entry_time_bucket) = 1 logical position
        ...
        for p in positions:
            ...
            # Skip duplicate chunks of the same split order
            logical_key = (p.symbol, p.type, p.time // 300)
    ...
    def _check_crypto(self, sym: str, direction: str, positions) -> bool:
        ...
        for p in positions:
            ...
            if is_crypto and _pos_direction(p) == direction:
                crypto_syms_same_dir.add(p_sym)
        same_dir = len(crypto_syms_same_dir)
    ...
    def _check_commodity(self, sym: str, direction: str, positions) -> bool:
        ...
        if is_metal:
            ...
            for p in positions:
                ...
                if p_sym in METAL_SYMBOLS and _pos_direction(p) == direction:
                    metal_syms.add(p_sym)
            same_dir = len(metal_syms)
        ...
        if is_oil:
            ...
            for p in positions:
                p_sym = _normalise(p.symbol)
                if p_sym in OIL_SYMBOLS and _pos_direction(p) == direction:
                    oil_syms.add(p_sym)
            same_dir = len(oil_syms)
            if same_dir >= MAX_METAL_SAME_DIR:  # same limit as metals (2)
                self.logger.log(
                    'WARNING', 'CorrelationGuard', 'OIL_CAP',
                    f'Blocked {sym} {direction}: already {same_dir} oil '
                    f'{direction}s (max {MAX_METAL_SAME_DIR})')
                return False
    ...
    def _check_index(self, sym: str, direction: str, positions) -> bool:
        ...
        for p in positions:
            ...
            if p_sym in INDEX_REGIONS[region] and _pos_direction(p) == direction:
                index_syms.add(p_sym)
        same_dir = len(index_syms)
    ...
    def _check_equity(self, sym: str, direction: str, positions) -> bool:
        ...
        for p in positions:
            ...
            if p_sym in EQUITY_SECTORS[sector] and _pos_direction(p) == direction:
                equity_syms.add(p_sym)
        same_dir = len(equity_syms)
