...
class OrderRouter:
    ...
    def execute_order_spec(self, spec) -> bool:
        """Execute a pre-validated OrderSpec from MasterOrderGenerator.

        This is a thin execution layer — guardrails already checked by Master.
        Only does: connectivity check, then delegates to execute_trade() which
        handles position sizing, order building, and sending.

        Strategy logic (SL/TP, sizing, all guardrails) is 100% identical to
        execute_trade() — we simply call it with the spec's parameters.
        """
        mt5 = self.mt5
        if mt5 is None:
            self.logger.log('ERROR', 'OrderRouter', 'MT5_DEAD',
                            f'Cannot execute {spec.symbol} — MT5 disconnected')
            return False

        # Connectivity guard — verify MT5 is actually responding
        try:
            test = mt5.account_info()
            if test is None:
                self.logger.log('ERROR', 'OrderRouter', 'MT5_NO_RESPONSE',
                                f'MT5 not responding for {spec.symbol}')
                return False
        except Exception as e:
            self.logger.log('ERROR', 'OrderRouter', 'MT5_ERROR',
