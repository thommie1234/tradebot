...
run_followup() {
    ...
    local GPU="$4"
    local WORKERS="$5"
    local ACCT_ID="$6"  # ftmo_100k or bright_100k
    local PREFIX="$7"   # ftmo or bf

    log "[$ACCT] Waiting for entry optuna PID=$ENTRY_PID..."

    # Wait for entry optuna to finish
    ...
    # Step 4a: Exit Optuna + sizing (200 trials, reduced from 400)
