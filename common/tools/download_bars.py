...
def download_symbol_costs(mt5: MT5BridgeClient, resolved: list[dict], out_dir: str) -> str:
    ...
    Real spread is computed from live bid/ask (not bar data points).
    """
    os.makedirs(out_dir, exist_ok=True)
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    fpath = os.path.join(out_dir, f"{today}_costs.csv")
    ...
    for sym_info in resolved:
        ...
        rows.append({
            "date": today,
            "symbol": mt5_name,
            "asset_class": sym_info.get("asset_class", ""),
            "bid": bid,
            "ask": ask,
    ...
    print(f"[download_bars] Cost snapshot: {fpath} ({len(rows)} symbols)")
    return fpath
...
def main():
    ...
    # Cost snapshot (spread, swap, commission per symbol)
    print(f"\n{'='*60}")
    print(f"  COST SNAPSHOT")
    print(f"{'='*60}")
    download_symbol_costs(mt5, resolved, COSTS_DIR)
