...
SESSION_FILE = str(REPO_ROOT / "common" / "tools" / "telegram_session")
SIGNAL_DB = str(REPO_ROOT / "bf" / "audit" / "telegram_signals.db")

# Default channels to monitor — top 3 from backtest
DEFAULT_CHANNELS = [
    "@FXNLofficial",    # FXNL Official — 22T, 50% WR, +5.5R
    -1848239554,        # Hobie's Trading Room — 23T, 52% WR, +5.7R
    -2740556781,        # FX Culture Free Signal Group — 18T, 44% WR, +4.1R
]
...
async def run_scraper(channels: list = None, account_id: str = "bright_100k",
    ...
    for ch in channels:
        try:
            if isinstance(ch, int):
                # Numeric IDs: try directly and with -100 prefix for supergroups
                for try_id in [ch, int(f"-100{abs(ch)}")]:
                    try:
                        entity = await client.get_entity(try_id)
                        monitored.append(entity.id)
                        title = getattr(entity, 'title', None) or getattr(entity, 'username', str(try_id))
                        print(f"[TELEGRAM] Monitoring: {title} (id={entity.id})")
                        break
                    except Exception:
                        continue
                else:
                    print(f"[TELEGRAM] WARNING: Cannot find channel id={ch}")
            else:
            ...
            print(f"[TELEGRAM] WARNING: Cannot find channel '{ch}': {e}")
    ...
    async def handler(event):
        ...
        if signal:
            ...
            print(f"  Entry: {signal.entry_low:.5f} - {signal.entry_high:.5f}")
            print(f"  Channel: {channel_name}")
            if signal.sl_price:
                print(f"  SL (ignored): {signal.sl_price}")
            if signal.tp_price:
                print(f"  TP (ignored): {signal.tp_price}")

            if dry_run:
                print(f"  [DRY RUN] Would execute {signal.direction} {signal.symbol}")
                _save_signal(SIGNAL_DB, signal, executed=False, result="DRY_RUN")
            else:
                # Random delay 5-30 seconds to avoid pattern detection
                import random
                delay = random.uniform(5, 30)
                print(f"  [DELAY] Waiting {delay:.0f}s before execution...")
                ...
                for acct in (account_id if isinstance(account_id, list) else [account_id]):
                    ...
                    print(f"  [{acct_label}] [{'OK' if ok else 'FAIL'}] {result}")
