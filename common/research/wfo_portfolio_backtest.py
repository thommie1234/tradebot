...
def backtest_symbol(symbol: str, exit_row: dict, args_dict: dict) -> dict:
    ...
    # Use sovereign_configs risk if available, else global default (0.5%)
    risk_pct = args_dict.get("sym_risk", {}).get(symbol) or args_dict.get("default_risk", 0.005)
...
def parse_args():
    ...
    p.add_argument("--risk", type=float, default=0.005,
                   help="Default risk per trade (0.005 = 0.5%%)")
    p.add_argument("--prob-threshold", type=float, default=0.0,
                   help="Override prob_threshold for all symbols (0=use per-symbol config)")
...
def main():
    ...
    # CLI override: apply same threshold to all symbols
    if args.prob_threshold > 0:
        sym_thresholds = {s: args.prob_threshold for s in symbols}

    args_dict = {
        "data_roots": DATA_ROOTS,
        "account_size": args.account_size,
        "z_threshold": 1.0,
        "prob_threshold": args.prob_threshold if args.prob_threshold > 0 else 0.55,
    ...
    print(f"[WFO] {len(symbols)} symbols | account=${args.account_size:,.0f} | {args.workers} workers | {mode} | risk={args.risk*100:.1f}%")
    if args.leak_free:
