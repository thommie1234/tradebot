...
def find_model_path(symbol: str) -> Path | None:
    # Check registry for versioned champion model
    ...
    if registry_path.exists():
        try:
            ...
            sym_entry = reg.get("symbols", {}).get(symbol, {})
            champion = sym_entry.get("champion")
            if champion:
                p = MODEL_DIR / "versions" / f"{champion}.json"
                if p.exists():
                    return p
        except Exception:
            pass
    ...
    for p in [MODEL_DIR / f"{symbol}.json",
        ...
        if p.exists():
            return p
    # Search versions directory
    versions_dir = MODEL_DIR / "versions"
    if versions_dir.exists():
        matches = sorted(versions_dir.glob(f"{symbol}_*.json"), reverse=True)
        if matches:
            return matches[0]
    return None
