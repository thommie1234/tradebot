...
class MultiTFScanner:
    ...
    def _should_scan(self, tf: str, minutes: int, now: datetime) -> bool:
        ...
        Scan at the bar close: minute 0 for H1, minute 0/15/30/45 for M15, etc.
        Add 5 seconds buffer for bar completion.
        """
        # Check if current minute is a bar boundary
        if now.minute % minutes != 0:
            return False

        # Only scan in the first 30 seconds after bar close
        if now.second > 30:
            return False

        # Don't scan if we already scanned this bar
        last = self._last_scan.get(tf, 0)
        if time.time() - last < minutes * 60 * 0.9:  # 90% of bar duration as cooldown
            return False
    ...
    def _scan_timeframe(self, tf: str, symbols: list[str], mt5) -> tuple[int, int]:
        ...
        self.bot.logger.log('INFO', 'MultiTF', 'SCAN_START',
                            f'{tf}: scanning {len(symbols)} symbols')
        ...
        for symbol in symbols:
            ...
            should_trade = confidence >= cfg.ML_THRESHOLD and direction is not None
            ...
            if not should_trade or direction is None:
                ...
                self.bot.logger.log('DEBUG', 'MultiTF', 'NO_SIGNAL',
                                    f'{symbol}[{tf}]: proba={confidence:.3f}{sent_info}')
                continue

            found += 1
            sent_info = f" sent={sent_boost:+.3f}" if sent_boost != 0 else ""
            self.bot.logger.log('INFO', 'MultiTF', 'SIGNAL',
                                f'{symbol}[{tf}] {direction} (proba={confidence:.3f}{sent_info})')
