...
def run_symbol(symbol: str, args_dict: dict) -> dict:
    ...
    if discovery_ev < 0 and len(study.trials) <= discovery_cutoff + 50:
        return {
            "symbol": symbol,
    ...
    # ── Phase 2: Deep dive (only for survivors with EV >= 0) ──
...
def main():
    ...
    print(f"[orchestrator] {len(symbols)} symbols | {args.timeframe} | "
