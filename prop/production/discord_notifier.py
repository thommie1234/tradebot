...
class DiscordNotifier:
    ...
    def trade_entry(self, symbol, direction, entry_price, lot_size, tp, sl, ticket, confidence):
        """Notify trade entry"""
        self.send(
    ...
    def trade_exit(self, symbol, direction, entry_price, exit_price, pnl, ticket, reason):
        """Notify trade exit"""
        color = 'green' if pnl > 0 else 'red'
        emoji = '✅' if pnl > 0 else '❌'

        self.send(
            title=f"{emoji} TRADE EXIT: {symbol} {direction}",
